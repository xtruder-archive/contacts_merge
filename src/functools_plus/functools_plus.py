...
__all__ = 'enum','struct','annotate','struct_fields', 'getannot', 'getannotbyid', 'setannot', 'hasannot', \
...
class struct():
	def __init__(self, *args):
		id=0
		for v in args[0]:
			setattr(self, getannotbyid(self, id), v)
			id+=1
...
def getannotbyid(obj, id, default=_NONE):
	if hasattr(obj, '__annotations__'):
		if default is _NONE:
			return obj.__annotations__.keys()[id]
	elif default is _NONE:
		raise KeyError(key)
	else:
		return default
